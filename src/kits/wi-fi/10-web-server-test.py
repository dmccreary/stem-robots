...
<body>
    <div class="container">
        ...
        <div class="section">
            <h3>🚗 Motor Control</h3>
            <div>
                <button class="motor-btn" onclick="sendCommand('move_forward')">⬆️ Forward</button>
                <button class="motor-btn" onclick="sendCommand('move_reverse')">⬇️ Reverse</button>
            </div>
            <div>
                <button class="motor-btn" onclick="sendCommand('turn_left')">⬅️ Turn Left</button>
                <button class="motor-btn" onclick="sendCommand('turn_right')">➡️ Turn Right</button>
            </div>
            <div>
    ...
    <script>
        function sendCommand(command) {{
            ...
            .catch(error => {{
                ...
                document.getElementById('last-command').textContent = 'Error: ' + command;
            }});
        }}
    </script>
</body>
        ...
        try:
            ...
            if 'GET / ' in request:
                ...
                if body.startswith('cmd='):
                    command = body[4:]  # Remove 'cmd=' prefix
