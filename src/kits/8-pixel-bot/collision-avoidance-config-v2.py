...
REVERSE_TIME = 1 # how long we backup
...
MOTOR_RIGHT_FORWARD_PIN = config.MOTOR_RIGHT_FORWARD_PIN
MOTOR_RIGHT_REVERSE_PIN = config.MOTOR_RIGHT_REVERSE_PIN
MOTOR_LEFT_FORWARD_PIN = config.MOTOR_LEFT_FORWARD_PIN
MOTOR_LEFT_REVERSE_PIN = config.MOTOR_LEFT_REVERSE_PIN

# our PWM objects
PWM_FREQUENCY = 50
right_forward = PWM(Pin(MOTOR_RIGHT_FORWARD_PIN))
right_forward.freq(PWM_FREQUENCY)

right_reverse = PWM(Pin(MOTOR_RIGHT_REVERSE_PIN))
right_reverse.freq(PWM_FREQUENCY)

left_forward = PWM(Pin(MOTOR_LEFT_FORWARD_PIN))
left_forward.freq(PWM_FREQUENCY)

left_reverse = PWM(Pin(MOTOR_LEFT_REVERSE_PIN))
...
NUMBER_PIXELS = 8
neopixel_pin = Pin(NEOPIXEL_PIN, Pin.OUT)   # set GPIO0 to output to drive NeoPixels
strip = NeoPixel(neopixel_pin, NUMBER_PIXELS)   # create NeoPixel driver on GPIO0 for 8 pixels

TOF_SDA_PIN = config.TOF_SDA_PIN
TOF_SCL_PIN = config.TOF_SCL_PIN
TOF_I2C_BUS = config.TOF_I2C_BUS

# I2C Setup
sda=machine.Pin(TOF_SDA_PIN) # row one on our standard Pico breadboard
scl=machine.Pin(TOF_SCL_PIN) # row two on our standard Pico breadboard
i2c=machine.I2C(TOF_I2C_BUS, sda=sda, scl=scl, freq=400000)
...
def main():
    ...
    while True:
        ...
        # wait until we get a hand wave in front of the bot
        if distance > TURN_DISTANCE and state == 0:
            if state != old_state:
                print('new state idel distance > ', distance)
            draw_idle(counter, .1)
            counter += 1
        if distance < TURN_DISTANCE and state == 0:
            state = 1
