#!/usr/bin/env bash
# Upload the whole 9-DOF IMU + display kit onto the Pico's flash
# filesystem: the shared l3gd20/lsm303dlhc drivers from src/lib/, the
# GC9B72 display driver + fonts from this kit's own lib/, config.py, every
# numbered lesson, and the two standalone diagnostic tools.
set -e
...
for name in gc9b72.py vga1_8x16.py vga1_bold_16x32.py; do
    echo "  lib/$name"
    mpremote connect "$PORT" fs cp "$SCRIPT_DIR/lib/$name" ":lib/$name"
done
...
for name in i2c-scanner-test.py board-info-test.py display-hello-test.py; do
    echo "  $name"
